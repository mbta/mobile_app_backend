defmodule MobileAppBackend.Notifications.StatsReporter do
  ...
  alias MobileAppBackend.Notifications.{DeliveredNotification, Subscription}
  ...
  def perform(%Oban.Job{}) do
    ...
    counts_by_route = Repo.all(from s in Subscription, group_by: [s.route_id], select: {s.route_id, count(s.id)})
    count_subscriptions_by_user = Repo.all(from s in Subscription, group_by: [s.user_id], select: count(s.id))
    ...
    Enum.each(counts_by_route, fn {route_id, count} ->
      ...
      Logger.info("#{__MODULE__} counts_by_route route_id=#{route_id} mode=#{mode} count=#{count}")
    end)

    Logger.info("#{__MODULE__} subscriptions_per_user user_count=#{user_count} \
