defmodule MobileAppBackendWeb.ClientConfigController do
  ...
  defp verify(conn, token) do
    ...
    jwk_response = Req.get(Application.get_env(:mobile_app_backend, MobileAppBackend.AppCheck)[:jwks_url], cache: true)
    ...
    with {:ok, %{body: %{"keys" => jwks}}} <- jwk_response,
      ...
      client_config = %ClientConfig{
        mapbox_token:
          Application.get_env(:mobile_app_backend, MobileAppBackend.ClientConfig)[:mapbox_token]
      }
