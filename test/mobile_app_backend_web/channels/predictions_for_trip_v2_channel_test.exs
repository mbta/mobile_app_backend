defmodule MobileAppBackendWeb.PredictionsForTripV2ChannelTest do
  ...
  alias MobileAppBackendWeb.PredictionsForTripV2Channel
  ...
  test "joins and leaves ok", %{socket: socket} do
    ...
    {:ok, reply, _socket} =
      subscribe_and_join(socket, "predictions:trip:v2:trip_1")

    assert reply == response
  end

  test "error if missing trip id in topic", %{socket: socket} do
    assert subscribe_and_join(socket, "predictions:trip:v2:") == {:error, %{code: :no_trip_id}}
  end
  ...
  test "handles new predictions", %{socket: socket} do
    ...
    PredictionsForTripV2Channel.handle_info(
