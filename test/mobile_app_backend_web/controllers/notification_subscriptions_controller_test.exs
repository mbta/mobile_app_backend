defmodule MobileAppBackendWeb.NotificationSubscriptionsControllerTest do
  ...
  describe "/accessibility with new FCM installation ID" do
    test "sets values of only the correct user", %{conn: conn} do
      ...
      conn =
        post(conn, "/api/notifications/subscriptions/accessibility", %{
          fcm_installation_id: user1.fcm_installation_id,
    ...
    test "correctly ignores unknown FCM installation ID", %{conn: conn} do
      conn =
        post(conn, "/api/notifications/subscriptions/accessibility", %{
  ...
  describe "/accessibility with old FCM token" do
    test "sets values of only the correct user", %{conn: conn} do
      [user1, user2] =
        insert_pair(:user_with_old_token,
          notification_subscriptions: fn ->
            build_pair(:notification_subscription, include_accessibility: true)
          end
        )
    ...
    test "sets locale", %{conn: conn} do
      user = insert(:user_with_old_token, locale: "en")
      assert user.locale == "en"
    ...
    test "refreshes FCM verification", %{conn: conn, now: now} do
      user = insert(:user_with_old_token)
  ...
  describe "/write with new FCM installation ID" do
  ...
  describe "/write with old FCM token" do
    test "creates new user with subscription", %{conn: conn, now: now} do
      conn =
        post(conn, "/api/notifications/subscriptions/write", %{
          fcm_token: "fake_token",
    ...
    test "removes subscription", %{conn: conn, now: now} do
      user =
        insert(:user_with_old_token,
          notification_subscriptions: [build(:notification_subscription)]
        )
