defmodule MobileAppBackendWeb.ScheduleControllerTest do
  ...
  describe "stop schedules" do
    ...
    test "when given multiple child stops only fetches once for parent", %{conn: conn} do
      s1 =
        %MBTAV3API.Schedule{
          id: "schedule-60565179-70159-90",
          arrival_time: ~B[2024-03-13 01:07:00],
      ...
      boyls = build(:stop, id: "place-boyls")
      stop1 = build(:stop, id: s1.stop_id, parent_station_id: boyls.id)
      stop2 = build(:stop, id: s2.stop_id, parent_station_id: boyls.id)
      ...
      |> expect(:get_data, 2, fn _ ->
        %{
          lines: %{},
          pattern_ids_by_stop: %{},
          routes: %{"Green-B" => %{type: :light_rail}},
          route_patterns: %{},
          stops: %{stop1.id => stop1, stop2.id => stop2, boyls.id => boyls},
          trips: %{}
        }
      end)
    ...
    test "when stop fetch times out, then cleanly returns error", %{conn: conn} do
      ...
      assert log =~ "fetch_schedules_parallel"
      assert log =~ "timeout"
    end
    ...
    test "does not filter ferry or cr schedules in the past", %{conn: conn} do
      ...
      |> expect(:get_data, 3, fn _ ->
        %{
          lines: %{},
          pattern_ids_by_stop: %{},
          routes: %{"CR-NewBedford" => %{type: :commuter_rail}, "Boat-F4" => %{type: :ferry}},
          route_patterns: %{},
          stops: %{},
          trips: %{}
        }
